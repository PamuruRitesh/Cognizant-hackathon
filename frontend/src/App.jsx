...
function App() {
  ...
  const loadHeaderData = () => {
    ...
    Promise.all([
      ...
      const dates = [...new Set((recRes.items || []).map(item => item.date).filter(Boolean))].sort()
            ...
            <div className="content-header">
              ...
              {showFilters && <div className="filter-strip">
                <button
                  ...
                  title={latestDate ? `Latest available date: ${latestDate}` : 'Loading available date'}
                >
                  <CalendarDays size={13} /> {latestDate || 'Today'}
                </button>
                {[
                  ...
                  <button
                    ...
                    title={value !== 'all' && count === 0 ? `No ${label.toLowerCase()} risk records in the current data` : `${count} matching record${count === 1 ? '' : 's'}`}
